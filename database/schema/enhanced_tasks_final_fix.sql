-- Enhanced Task Management System - FINAL SAFE MIGRATION
-- This script safely handles existing tables and data with fixed syntax
...
DO $$
BEGIN
    -- Add estimated_hours constraint
    BEGIN
        ALTER TABLE tasks ADD CONSTRAINT tasks_estimated_hours_check CHECK (estimated_hours >= 0);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
    
    -- Add actual_hours constraint
    BEGIN
        ALTER TABLE tasks ADD CONSTRAINT tasks_actual_hours_check CHECK (actual_hours >= 0);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
    
    -- Add story_points constraint
    BEGIN
        ALTER TABLE tasks ADD CONSTRAINT tasks_story_points_check CHECK (story_points >= 0 AND story_points <= 100);
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
    ...
    EXCEPTION 
        ...
        WHEN check_violation THEN 
            UPDATE tasks SET effort_level = 'moderate' WHERE effort_level NOT IN ('minimal', 'light', 'moderate', 'heavy', 'intensive');
            ALTER TABLE tasks ADD CONSTRAINT tasks_effort_level_check 
            CHECK (effort_level IN ('minimal', 'light', 'moderate', 'heavy', 'intensive'));
    END;
    ...
    EXCEPTION 
        WHEN duplicate_object THEN NULL;
        WHEN check_violation THEN 
            UPDATE tasks SET complexity = 'moderate' WHERE complexity NOT IN ('simple', 'moderate', 'complex', 'very_complex');
            ALTER TABLE tasks ADD CONSTRAINT tasks_complexity_check 
            CHECK (complexity IN ('simple', 'moderate', 'complex', 'very_complex'));
    END;
    
    -- Add risk_level constraint
    BEGIN
    ...
    EXCEPTION 
        WHEN duplicate_object THEN NULL;
        WHEN check_violation THEN 
            UPDATE tasks SET risk_level = 'low' WHERE risk_level NOT IN ('low', 'medium', 'high', 'critical');
            ALTER TABLE tasks ADD CONSTRAINT tasks_risk_level_check 
            CHECK (risk_level IN ('low', 'medium', 'high', 'critical'));
    END;
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'task_comments') THEN
        ...
        UPDATE task_comments SET comment_type = 'comment' WHERE comment_type IS NULL;
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tasks_custom_fields') THEN
...
UPDATE tasks SET effort_level = 'moderate' WHERE effort_level IS NULL;
UPDATE tasks SET complexity = 'moderate' WHERE complexity IS NULL;
UPDATE tasks SET risk_level = 'low' WHERE risk_level IS NULL;
...
UPDATE tasks SET custom_fields = '{}' WHERE custom_fields IS NULL;
