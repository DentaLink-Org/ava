-- Migration: Add milestone management tables
-- This migration adds comprehensive milestone functionality to the existing task system
-- Execute order: 002 (after 000_create_migrations_table and 001_add_category_to_page_components)

-- Record migration start
INSERT INTO migrations (filename, executed_at, success) 
VALUES ('002_add_milestone_tables.sql', NOW(), FALSE)
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'tasks' AND column_name = 'milestone_id') THEN
...
DROP TRIGGER IF EXISTS update_milestone_comments_updated_at ON milestone_comments;
CREATE TRIGGER update_milestone_comments_updated_at BEFORE UPDATE ON milestone_comments
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS milestone_progress_on_task_update ON tasks;
CREATE TRIGGER milestone_progress_on_task_update
...
DROP TRIGGER IF EXISTS milestone_progress_on_task_insert ON tasks;
CREATE TRIGGER milestone_progress_on_task_insert
    AFTER INSERT ON tasks
    FOR EACH ROW
    WHEN (NEW.milestone_id IS NOT NULL)
    EXECUTE FUNCTION update_milestone_progress_on_task_change();

DROP TRIGGER IF EXISTS milestone_progress_on_task_delete ON tasks;
CREATE TRIGGER milestone_progress_on_task_delete
...
DROP TRIGGER IF EXISTS prevent_circular_milestone_dependencies ON milestone_dependencies;
CREATE TRIGGER prevent_circular_milestone_dependencies
...
DROP POLICY IF EXISTS "Allow public access to team_members" ON team_members;
CREATE POLICY "Allow public access to team_members" ON team_members FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow public access to milestones" ON milestones;
CREATE POLICY "Allow public access to milestones" ON milestones FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow public access to milestone_dependencies" ON milestone_dependencies;
CREATE POLICY "Allow public access to milestone_dependencies" ON milestone_dependencies FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow public access to milestone_progress" ON milestone_progress;
CREATE POLICY "Allow public access to milestone_progress" ON milestone_progress FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow public access to milestone_comments" ON milestone_comments;
...
WHERE filename = '002_add_milestone_tables.sql';
...
-- Verification query
SELECT 
    'Milestone migration completed successfully!' as status,
